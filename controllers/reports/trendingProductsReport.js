...
exports.getFilterData = async (req, res) => {
  try {
    ...
    const [categories, subcategories, brands, locations, units, productTypes] = await Promise.all([
      Category.find({ parentId: null, isDeleted: { $ne: true } })
        .select('name')
        .sort({ name: 1 }),
      
      Category.find({ parentId: { $ne: null }, isDeleted: { $ne: true } })
      ...
      Product.distinct('unit'),
      
      Product.distinct('type')
    ]);

    // Format and return the filter data
    res.status(200).json({
